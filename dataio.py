...
def get_mgrid_rect(sidelen, dim=3):
    '''Generates a flattened grid of (x,y,...) coordinates in a range of -1 to 1.'''
    if dim == 1:
    ...
    pixel_coords = torch.squeeze(pixel_coords, 0)
...
class Implicit3DWrapper(torch.utils.data.Dataset):
    def __init__(self, dataset, sidelength=None, sample_fraction=1.):
        ...
        if isinstance(sidelength[0], int):
...
class Detector(Dataset): # Current Detector geometry is (74, 77, 394, 180)
    def __init__(self, detector):
        super().__init__()
        self.detector = detector
        self.shape = self.detector.shape[:-1]
        self.channels = self.detector.shape[-1]
...
class PhotonWrapper(torch.utils.data.Dataset):
    def __init__(self, dataset, sidelength, sample_fraction=1.):

        self.dataset = dataset
        self.mgrid = get_mgrid_rect(sidelength, dim=3)
        self.data = (torch.from_numpy(self.dataset[0]) - 0.5) / 0.5
        self.sample_fraction = sample_fraction
        self.N_samples = int(self.sample_fraction * self.mgrid.shape[0])
